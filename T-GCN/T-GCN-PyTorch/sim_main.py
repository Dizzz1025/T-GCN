...
DATA_PATHS = {
    "sim": {"feat": "data/sim/merged_interp_xy_norm.pkl", "adj": "data/sim/edges_train_merged.npy"},
}
...
def get_task(args, model, dm):
    task = getattr(tasks, "Sim" + args.settings.capitalize() + "ForecastTask")(
...
def main_supervised(args):
    dm = utils.data.SimSpatioTemporalPklDataModule(
        feat_path=DATA_PATHS[args.data]["feat"], edges_path=DATA_PATHS[args.data]["adj"], **vars(args)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data", type=str, help="The name of the dataset", choices=("sim"), default="sim")
    ...
    parser = getattr(utils.data.data_sim, "SimSpatioTemporalPklDataModule").add_data_specific_arguments(parser)
